...
def inference(x_tensor, args, mode):
  ...
  with tf.variable_scope('fc5') as scope:
    weights = _variable_with_weight_decay('weights',
					  ...
					  wd=0.0005)
    biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.1))
    fc5 = tf.nn.relu(tf.matmul(dropout4, weights) + biases, name=scope.name)
...
def train(total_loss, global_step, args):
  ...
  lr = tf.train.exponential_decay(args.INITIAL_LEARNING_RATE, global_step, decay_steps, args.LEARNING_RATE_DECAY_FACTOR, staircase=True)
